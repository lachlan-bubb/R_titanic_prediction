...
features <- c("age_banded", "male", "Pclass", "Parch", "SibSp")

dtrain <- xgb.DMatrix(data = data.matrix(train[holdout_flag %in% c(0, 1), ..features]), 
                      label = data.matrix(train[holdout_flag %in% c(0, 1), response]))
...
# xgb.plot.shap(data  = data.matrix(train[holdout_flag == 0, .(Age, male)]), 
#               model = xgb)
